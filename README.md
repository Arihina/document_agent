...
`MINERU_BACKEND`/`MINERU_LANG` — то, что подтверждено на этапе локального
прогона MinerU (CPU-совместимый `pipeline`, явная кириллическая модель, а не
общая мультиязычная — по тем же причинам, что и в OCR-агенте). Меняется без
правки кода, если для другого класса документов понадобится другой бэкенд.

Если MinerU недоступен на старте — сервис всё равно поднимется (это не
фатально), просто разбор документов будет отвечать `502`, пока MinerU не
появится. Проверяется в `main.py` при старте, пишется в лог.
